...
from app.agent.runner.new_input import run_new_input_agent
from app.agent.runner.text import candidate_query_text
from app.agent.runner.thread_followup import run_thread_followup
...
async def process_raw_input(session: Session, raw_input_id: uuid.UUID) -> dict:
    ...
    # --- 2. Embed once -------------------------------------------------------
    query_text = candidate_query_text(raw.content, meta)
