...
from app.services.github import issues as github_issues  # noqa: E402
...
@pytest.mark.asyncio
async def test_create_task_issue_maps_csee_to_repo_and_payload(monkeypatch):
    ...
    class FakeResponse:
        ...
        def json(self):
            return {"html_url": "https://github.com/askLio/CSEE-strategic-negotiation-agent/issues/7"}
    ...
    monkeypatch.setattr(github_issues, "get_settings", lambda: SimpleNamespace(github_token="ghp_test"))
    monkeypatch.setattr(github_issues.httpx, "AsyncClient", FakeAsyncClient)

    issue = await github_issues.create_task_issue(task)

    assert issue.html_url == "https://github.com/askLio/CSEE-strategic-negotiation-agent/issues/7"
    assert calls == [
        {
            "url": "https://api.github.com/repos/askLio/CSEE-strategic-negotiation-agent/issues",
            "json": {
                "title": "Implement task action",
...
@pytest.mark.asyncio
async def test_create_task_issue_maps_social_ai_to_repo(monkeypatch):
    ...
    class FakeResponse:
        ...
        def json(self):
            return {"html_url": "https://github.com/TUM-Social-AI/AflaConnect/issues/5"}
    ...
    await github_issues.create_task_issue(task)

    assert calls[0][0] == "https://api.github.com/repos/TUM-Social-AI/AflaConnect/issues"
...
async def test_create_github_issue_denies_existing_github_link(monkeypatch):
    ...
    async def fake_create_task_issue(_task):
        nonlocal called
        called = True

    monkeypatch.setattr(tasks_api.tasks_store, "get", lambda *_args: task)
    monkeypatch.setattr(tasks_api, "create_task_issue", fake_create_task_issue)
...
async def test_create_github_issue_stores_url_commits_and_publishes(monkeypatch):
    ...
    async def fake_create_task_issue(_task):
        return SimpleNamespace(html_url="https://github.com/TUM-Social-AI/AflaConnect/issues/5")
