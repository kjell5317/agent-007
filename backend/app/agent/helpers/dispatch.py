...
by acting on an *existing* task: patch it (`update_task`), mark it done
(`close_task`), or leave it alone (`no_change`). The side effects are
identical — reschedule/notify on update, notify on close — so they live here
and both runners call the same function.
"""
...
# Fields carried on the new-input tool variants that aren't task columns and
# must be stripped before patching.
_NON_PATCH_FIELDS = frozenset({"existing_task_id", "reason", "confidence"})


async def apply_task_action(
    session: Session, task: Task, tool_name: str, tu_input: dict[str, Any]
) -> dict[str, Any]:
    """Apply `update_task` / `close_task` / `no_change` to `task`.
    ...
    if tool_name == "update_task":
        ...
        # own "Rescheduled" notification with the new slot. Firing the generic
        # "Agent updated" too would replace it via the shared task tag and lose
        # the slot, so only notify for non-plan edits.
    ...
    if tool_name == "close_task":
        ...
        return {"outcome": "closed", "confidence": tu_input.get("confidence")}
